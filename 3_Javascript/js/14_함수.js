...
document.querySelector("#btn2a").addEventListener("click", function () {
  ...
  // -> 함수 수행 후 돌아와서도
  // 참조되어 있던 배열의 번 인덱스 값이 변화되어 있음

  // 참조란?
  /**
   * - 객체 지향 언어에서
   * 배열, 객체 등 값을 여러개 가지고 있는 자료형인데 변수는
   * 값을 1개만 저장할 수 있기 때문에
   * 
...
document.querySelector("#btn3a").addEventListener("click", function () {
  ...
  numbers.push(50);
  numbers.push(4);
  numbers.push(returnTest(2, 5)); // 함수를 호출해서 되돌려 받은 값(return 값;)을 push 하기
...
const arrowList = document.querySelectorAll(".arrow"); // 유사배열 형태
...
document.getElementById("button1a").addEventListener("click", () => {
  ...
  button1a.style.backgroundColor=input1a.value;
