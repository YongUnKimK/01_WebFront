...
function startGame() {
  ...
  // 정답누른 횟수 
  let attempt = 1;
  // prompt에 입력한 값을 저장할 함수
  console.log(answer)
  let answer1 = " 1부터 200사이 숫자 입력 ";
  while(answer != answer1 ) {  
  
    let answer1 = prompt("값을 입력해 주세요");  
    if(answer1 === null) {
      alert("취소되었습니다"); break;
    }
    else {
      if(answer1 == answer) {
        alert(`${answer}!정답입니다! 시도횟수 : ${attempt++}`); break;
      }
      if(isNaN(answer1) == true) {
        alert("숫자를 제외한 다른 문자는 입력할 수 없습니다"); attempt++; continue;
      }
      if(answer1 < 1 || answer1 > 200) {
        alert("1~200사이 값만 작성해주세요") ; attempt++; ;continue;
      }
      
        if(answer1 < answer) {
          alert(`Up 입니다 더 높은 수를 입력하세요. 시도횟수 ${attempt}` );
          attempt++ ; continue;
        }
        else {
          alert(`Down 입니다 더 낮은 수를 입력하세요. 시도횟수 ${attempt}`); 
          attempt++ ; continue;
        }
