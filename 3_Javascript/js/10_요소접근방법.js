...
function classTest() {
  ...
  // divs 0, 1, 2쨰 요소에 배경색 변경
  divs[0].style.backgroundColor = "rgb(114,203,80)" ;
  divs[1].style.backgroundColor = "rgb(5,23,80)" ;
  divs[2].style.backgroundColor = "rgb(38,84,80)" ;
}
...
function nameTest() {
  ...
  for(let i = 0; i < hobbyList.length; i++) {
    ...
    // input요소.checked = false - > 해당 요소를 체크하지 않겠다.
...
function cssTest() {
  ...
  // -> 선택자가 선택한 요소 중 첫번째 요소만 반환

  // [요소 n개 -> 배열]
  // 2) document.querySelectorAll("css 선택자")
  // -> 선택한 요소 모두를 배열(유사 배열)형태로 변환
...
function readValue() {
  ...
  //  올라온 키가 enter키이면 realValue()함수를 호출

  // keyup : 눌러지던 키가 떼어졌을때 ( 1회만 인식 )
  // keydown : 키가 꾹 눌러질떄 ( + 꾹 누르고 있으면 계속 인식됨)
  // e : 이벤트 객체(발생한 이벤트 정보를 담고 있는 )
}
