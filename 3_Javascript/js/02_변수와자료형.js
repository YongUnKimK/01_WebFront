...
console.log(1234);
console.log("문자열은 양쪽에 쌍따옴표");
console.log('JS는 홑따옴표도 문자열로 취급');
//console.log(문자열); 변수로 취급
...
console.log("100" + 100) ; //= 100100 [문자열 + 숫자열의 형태]
...
let number3 = 25;
//let number3 = 500; 빨간줄 에러나욤~
// 같은 이름으로(변수명) 선언을 또다시 하려고 해서 에러가 발생하였음.
console.log(number3)
...
// 중복 선언 불가
// 선언만 할 수 없음 -> 선언 및 초기화가 동시에 이루어져야 함.

const PI = 3.141592;
//PI = 1.23;  TypeError : Assignment to constant variable.
// -> 상수에 새로운 값을 대입할 수 없는데, 대입하려고 해서 에러가 발생
...
//선언과 호출 세트.. 선언을 했으면 호출을 반드시 해라~!!@_@

//console.log(test3); //ReferenceError: num1 is not defined
...
/* TDZ ( Temporal Dead Zone ) 
  ...
  - Javascript에서 let/const 키워드로 선언된 변수가 초기화되기 전까지 접근할 수 없는 구간을 의미함.
...
//console.log(x); -> Uncaught ReferenceError: Cannot acces 

let x = 50; // 변수의 선언과 초기화

console.log(x); //50

// 이러한 특성들 때문에 let/const는 var보다 예측 가능한
// 코드를 작성할 수 있다.
...
// 1) 배열(Array) : 여러 값이 나열되어있는 것의 묶음입니다
// Js의 배열은 []로 작성
const numbers = [10, 20, 30]; 

// index : 배열안의 요소들을 지칭 [0부터 시작]
...
// 2) JS 객체
// 값을 K:V (Map) 형식으로 여러개 저장하는 형태 [키밸류]
// -> { K:V, K:V, K:V ...}
...
// 함수를 호출할 떄 전달하는 값 : 전달인자(Argument)